...
const insertAnimes:any = express.Router();
...
insertAnimes.post("/",upload.single("image"),async (req:any,res:any):Promise<any>=>{
    ...
    try{
        ...
        const result:any = await db.collection("animes").insertOne({
            "id" : req.body.id,
            "name" : req.body.name,
