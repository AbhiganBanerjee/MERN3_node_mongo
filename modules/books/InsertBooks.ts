...
const insertBooks:any = express.Router();
...
insertBooks.post("/",upload.single("image"),async (req:any,res:any):Promise<any>=>{
    ...
    try{
        ...
        const result:any = await db.collection("books").insertOne({
            "id" : req.body.id,
            "name" : req.body.name,
