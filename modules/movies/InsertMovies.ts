...
const insertMovies:any = express.Router();
...
insertMovies.post("/",upload.single("image"),async (req:any,res:any):Promise<any>=>{
    ...
    try{
        ...
        const result:any = await db.collection("movies").insertOne({
            "id" : req.body.id,
            "name" : req.body.name,
            "type" : req.body.type,
