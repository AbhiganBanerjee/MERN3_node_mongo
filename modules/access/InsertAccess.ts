...
const insertAccess:any = express.Router();
...
insertAccess.post("/",upload.single("image"),async (req:any,res:any):Promise<any>=>{
    ...
    try{
        ...
        const result:any = await db.collection("access").insertOne({
            "id" : req.body.id,
            "name" : req.body.name,
